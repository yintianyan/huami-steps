...
async function manualRun(){
  const input=prompt('输入要增加的步数（留空则自动执行一次）:', '');
  if(input===null) return; // 取消
  const steps=parseInt(input);
  if(input!==''&&(isNaN(steps)||steps<0)){toast('请输入有效步数',false);return}
  try{
        ...
        try{
          ...
          if(manualSteps>0){
            ...
            for(const acct of cfg.accounts){
              ...
              const newTotal=cur+manualSteps;
            ...
            jsonRes(res,{ok:true,msg:`已提交 +${manualSteps.toLocaleString()} 步`});
          }else{
